...
def pregnancy_details_view(request):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            prompt = (
                "Analyze the following patient profile and lab reports. Provide a concise, 2-3 line summary "
...
def chat(request):
    ...
    if latest_report_obj and latest_report_obj.data:
        ...
        context_summary = (
            f"- Patient is {data.get('weeks_pregnant', 'unknown')} weeks pregnant. "
            f"- Conditions: {', '.join([k for k, v in data.get('conditions', {}).items() if v]) or 'None'}. "
        )
    system_prompt = (
        f"You are Shrishti.ai, a friendly and empathetic AI pregnancy companion. You are NOT a doctor. "
        f"Respond in {lang}. Always remind the user to consult their doctor for any medical concerns. "
        f"Use the following context: {context_summary}. Keep answers concise and caring (2-4 sentences). "
        f"For medical questions, give a general answer and ALWAYS end with "
        f"'Please consult your doctor for personalized medical advice.'"
    )
...
def send_frame(request):
    ...
    try:
        ...
        base = (
            ...
            "- If there IS a user question: answer it in 1–3 sentences grounded in the image.\n"
            "- If there is NO question: give a 1–2 line friendly description of what's in the image.\n"
            "If you are unsure, say so. Avoid medical directives; for medical issues end with: "
...
def send_audio(request):
    ...
    try:
        ...
        base_prompt = (
            "You are Shrishti.ai, an empathetic pregnancy companion. "
            "Listen to the user's audio. If they asked a question, answer it in 1–3 sentences. "
            "If it's general speech, summarize it in one friendly line and ask a gentle follow-up. "
            "If it sounds medical, keep it general and end with: "
            "'Please consult your doctor for personalized medical advice.'"
        )
        if user_hint:
            base_prompt += f"\nUser hint: {user_hint}"
